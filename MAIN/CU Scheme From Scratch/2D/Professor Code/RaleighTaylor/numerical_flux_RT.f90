    subroutine numerical_flux_RT(rho,mx,my,E,newdtneed,F1,F2,F3,F4,G1,G2,G3,G4)
    implicit none 
    ...
    do j=1,Ny
        do i=1,Nx
            ...
            if (j==1) then
                d1=(rho(i,j+1)-rho(i,j))
                d2=2.d0*(rho(i,j)-2.d0)
                sly=0.5d0*minmod(d1,d2) 
            elseif (j==Ny) then
                d1=(rho(i,j)-rho(i,j-1))
                d2=2.d0*(1.d0-rho(i,j))
                sly=0.5d0*minmod(d1,d2) 
            else
            ...
            if (j==1) then
                d1=(E(i,j+1)-E(i,j))
                d2=2.d0*(E(i,j)-1.d0/(gamma-1.d0))
                sly=0.5d0*minmod(d1,d2) 
            elseif (j==Ny) then
                d1=(E(i,j)-E(i,j-1))
                d2=2.d0*(2.5d0/(gamma-1.d0)-E(i,j))
                sly=0.5d0*minmod(d1,d2) 
            else
    ...
    do i=1,Nx
        ...
        mxN(i,0)=0.d0
        myN(i,0)=0.d0
        EN(i,0)=1.d0/(gamma-1.d0)
        rhoS(i,Ny+1)=1.d0
        mxS(i,Ny+1)=0.d0
        myS(i,Ny+1)=0.d0
        ES(i,Ny+1)=2.5d0/(gamma-1.d0)
        ...
        myNE(i,0)=0.d0
        myNW(i,0)=0.d0
        ESW(i,Ny+1)=2.5d0/(gamma-1.d0)
        ESE(i,Ny+1)=2.5d0/(gamma-1.d0)
        ENE(i,0)=1.d0/(gamma-1.d0)
        ENW(i,0)=1.d0/(gamma-1.d0)
    enddo
