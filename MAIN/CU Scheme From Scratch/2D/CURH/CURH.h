...
vvvvd CURH::curh_flux(vvd rho, vvd mx, vvd my, vvd E, bool update_time){
    ...
    /*
        !STEP-1 : Calculation of Piecewise Linear Reconstruction
    */
    ...
    /*
        !STEP-2 : Local Speeds of Propagation in x-direction
    */
    vvd ap(row, vd(col,0));
    vvd am(row, vd(col,0));

    // CURH Numerical flux vectors
    vvd F1(row, vd(col,0));
    vvd F2(row, vd(col,0));
    vvd F3(row, vd(col,0));
    vvd F4(row, vd(col,0));
    ...
    for(int i=1 ; i<row-1 ; i++){
        for(int j=0 ; j<col-1 ; j++){
            ...
            if( dist > CTS::EPSILON ){
                F1[i][j] = ( (ap[i][j]*f1E - am[i][j]*f1W) + prod*( rho_plr[3][i][j+1] - rho_plr[2][i][j] ) ) / dist;
                F2[i][j] = ( (ap[i][j]*f2E - am[i][j]*f2W) + prod*( mx_plr[3][i][j+1] - mx_plr[2][i][j] ) ) / dist;
                F3[i][j] = ( (ap[i][j]*f3E - am[i][j]*f3W) + prod*( my_plr[3][i][j+1] - my_plr[2][i][j] ) ) / dist;
                F4[i][j] = ( (ap[i][j]*f4E - am[i][j]*f4W) + prod*( E_plr[3][i][j+1] - E_plr[2][i][j] ) ) / dist;
            }
            else{
                F1[i][j] = 0.5*( f1E + f1W );
                F2[i][j] = 0.5*( f2E + f2W );
    ...
    /*
        !STEP-2 : Local Speeds of Propagation in y-direction
    */
    vvd bp(row, vd(col,0));
    vvd bm(row, vd(col,0));
    ...
    for(int i=0 ; i<row-1 ; i++){
        for(int j=1 ; j<col-1 ; j++){
            ...
            if( dist > CTS::EPSILON ){
                G1[i][j] = ( (bp[i][j]*g1N - bm[i][j]*g1S) + prod*( ( rho_plr[1][i+1][j] - rho_plr[0][i][j] ) ) ) / dist;
                G2[i][j] = ( (bp[i][j]*g2N - bm[i][j]*g2S) + prod*( ( mx_plr[1][i+1][j] - mx_plr[0][i][j] ) ) ) / dist;
                G3[i][j] = ( (bp[i][j]*g3N - bm[i][j]*g3S) + prod*( ( my_plr[1][i+1][j] - my_plr[0][i][j] ) ) ) / dist;
                G4[i][j] = ( (bp[i][j]*g4N - bm[i][j]*g4S) + prod*( ( E_plr[1][i+1][j] - E_plr[0][i][j] ) ) ) / dist;
            }  
            else{
                G1[i][j] = 0.5*( g1N + g1S );                
                G2[i][j] = 0.5*( g2N + g2S );                
    ...
    /*
        ! Numerical flux vectors to be returned by the function
    */
    vvvd F = {F1, F2, F3, F4};
    vvvd G = {G1, G2, G3, G4};

    /*
        ! Time step updation using CFL conditions
    */
    if( update_time ){
        double amax=0.0;
        double bmax=0.0;
