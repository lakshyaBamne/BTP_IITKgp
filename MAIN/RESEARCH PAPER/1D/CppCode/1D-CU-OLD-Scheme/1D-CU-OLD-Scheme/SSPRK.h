...
vector<vector<double>> SSPRK::get_next_cons_vars(vector<vector<double>>& cons_vars, vector<vector<double>>& cu_flux, double LAMBDA, pair<string,string>& initial_conditions, double& dt, double& dx, double& t, pair<double,double>& time){
    ...
    vector<vector<double>> cu_flux_u1;

    if( CTS::FLUX == "LLF" ){
        cu_flux_u1 = CUF::get_cu_flux_llf(U1, initial_conditions, dt, dx, t, time);
    }
    else if( CTS::FLUX == "CU-OLD" ){
        cu_flux_u1 = CUF::get_cu_flux(U1, initial_conditions, dt, dx, t, time);
    }
    else if( CTS::FLUX == "HLL" ){
        cu_flux_u1 = CUF::get_cu_flux_hll(U1, initial_conditions, dt, dx, t, time);
    }
    else{
        cout << "---ERROR--- Please enter correct flux type---" << endl;
    }
    ...
    // ! we don't want to calculate a new time step here so we have to keep a copy of the old one
    vector<vector<double>> cu_flux_u2;

    if( CTS::FLUX == "LLF" ){
        cu_flux_u2 = CUF::get_cu_flux_llf(U2, initial_conditions, dt, dx, t, time);
    }
    else if( CTS::FLUX == "CU-OLD" ){
        cu_flux_u2 = CUF::get_cu_flux(U2, initial_conditions, dt, dx, t, time);
    }
    else if( CTS::FLUX == "HLL" ){
        cu_flux_u2 = CUF::get_cu_flux_hll(U2, initial_conditions, dt, dx, t, time);
    }
    else{
        cout << "---ERROR--- Please enter correct flux type---" << endl;
    }
