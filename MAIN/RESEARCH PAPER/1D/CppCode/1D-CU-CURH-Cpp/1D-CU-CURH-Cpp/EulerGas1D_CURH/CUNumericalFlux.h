...
namespace CUNumericalFlux{
    ...
    // CURH optimization block - A sharper estimate for the local speeds of propagation
    void curh(vector< vector<double> > &lsp, vector< vector< vector<double> > > plr, vector< vector< vector<double> > > flux);
...
vector<vector<double>> CUNumericalFlux::get_cu_flux( vector<vector<double>>& cons_vars, pair<string,string>& initial_conditions, double& dt, double& dx, double& t, pair<double,double>& time){
    ...
    // CURH Optimization - A sharper estimate for the one sided local speeds of propagation
    curh(lsp, {rho_plr, m_plr, E_plr}, {f1, f2, f3});
...
// CURH optimization - A sharper estimate for the local speeds of propagation
void CUNumericalFlux::curh(vector< vector<double> > &lsp, vector< vector< vector<double> > > plr, vector< vector< vector<double> > > flux){
    vector< vector<double> > lsp2 = lsp;
    
    // first get the required variables
    vector< vector<double> > rho_plr = plr[0];
    vector< vector<double> > m_plr = plr[1];
    vector< vector<double> > E_plr = plr[2];

    vector< vector<double> > f1 = flux[0];
    vector< vector<double> > f2 = flux[1];
    vector< vector<double> > f3 = flux[2];

    ll len = rho_plr[0].size();

    double df1, df2, df3;
    double du1, du2, du3;
    double du1eps, du2eps, du3eps;
    double ratio1, ratio2, ratio3;
    double ratiomin, ratiomax;

    // now we need to optimize the local speeds of propagation using the RH conditions
    for(ll i=0 ; i<len-1 ; i++){
        df1 = f1[1][i+1] - f1[0][i];
        df2 = f2[1][i+1] - f2[0][i];
        df3 = f3[1][i+1] - f3[0][i];
    
        du1 = rho_plr[1][i+1] - rho_plr[0][i];
        du2 = m_plr[1][i+1] - m_plr[0][i];
        du3 = E_plr[1][i+1] - E_plr[0][i];
    
        if( du1 > -1*CTS::EPSILON ){
            du1eps = max(du1, CTS::EPSILON);
        }
        else{
            du1eps = min(du1, -1*CTS::EPSILON);
        }

        if( du2 > -1*CTS::EPSILON ){
            du2eps = max(du2, CTS::EPSILON);
        }
        else{
            du2eps = min(du2, -1*CTS::EPSILON);
        }

        if( du3 > -1*CTS::EPSILON ){
            du3eps = max(du3, CTS::EPSILON);
        }
        else{
            du3eps = min(du3, -1*CTS::EPSILON);
        }

        ratio1 = (2.0 * df1) / (du1 + du1eps);
        ratio2 = (2.0 * df2) / (du2 + du2eps);
        ratio3 = (2.0 * df3) / (du3 + du3eps);

        ratiomin = min(ratio1, min(ratio2, ratio3));
        ratiomax = max(ratio1, max(ratio2, ratio3));

        if( ratiomax > -1*CTS::EPSILON ){
            lsp[0][i] = min(lsp2[0][i], ratiomax);
            lsp[1][i] = max(lsp2[1][i], -1*ratiomax);
        }

        if( ratiomin < CTS::EPSILON ){
            lsp[0][i] = min(lsp2[0][i], -1*ratiomin);
            lsp[1][i] = max(lsp2[1][i], ratiomin);
        }
    }
