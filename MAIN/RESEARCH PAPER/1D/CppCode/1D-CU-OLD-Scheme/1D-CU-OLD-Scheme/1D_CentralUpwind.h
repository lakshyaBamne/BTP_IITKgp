...
public: // methods
    ...
    void run_cu_scheme_partial(string result){
        //! Initialize conserved variables on the computational grid and extend boundaries
        ...
        while( t < time.second ){
            ...
            if( CTS::FLUX == "LLF" ){
                cu_flux = CUF::get_cu_flux_llf(cons_vars, initial_conditions, dt, dx, t, time);
            }
            else if( CTS::FLUX == "CU-OLD" ){
                cu_flux = CUF::get_cu_flux(cons_vars, initial_conditions, dt, dx, t, time);
            }
            else if( CTS::FLUX == "HLL" ){
                cu_flux = CUF::get_cu_flux_hll(cons_vars, initial_conditions, dt, dx, t, time);
            }
            else{
