/*
    Code for 1st Order Lax Friedrichs Scheme
*/
...
vvd get_lf_flux(vvd& U, vvd& F, double dt);
...
vvd get_lf_flux(vvd& U, vvd& F, double dt){
    ...
    for(int i=0 ; i<=CTS::N ; i++){
        LF[0][i] = 0.5*(F[0][i]+F[0][i+1]) - 0.5*(CTS::dx/dt)*(U[0][i+1]-U[0][i]);
        LF[1][i] = 0.5*(F[1][i]+F[1][i+1]) - 0.5*(CTS::dx/dt)*(U[1][i+1]-U[1][i]);
        LF[2][i] = 0.5*(F[2][i]+F[2][i+1]) - 0.5*(CTS::dx/dt)*(U[2][i+1]-U[2][i]);
    }
