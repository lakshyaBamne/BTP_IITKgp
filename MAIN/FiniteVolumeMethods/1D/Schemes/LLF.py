"""
    Module to implement the Local Lax Friedrichs (LLF) Scheme for
...
def fvm1d_llf(RP: dict) -> None:
    ...
    for i in range(RP["N"]+1):
        lamda = max(abs(RP["u"][i])+RP["a"][i], abs(RP["u"][i+1])+RP["a"][i+1])
        LF1[i] = 0.5*(F[0][i]+F[0][i+1]) - 0.5*lamda*(RP["U"][0][i+1] - RP["U"][0][i])
        LF2[i] = 0.5*(F[1][i]+F[1][i+1]) - 0.5*lamda*(RP["U"][1][i+1] - RP["U"][1][i])
        LF3[i] = 0.5*(F[2][i]+F[2][i+1]) - 0.5*lamda*(RP["U"][2][i+1] - RP["U"][2][i])
...
def run_llf(PROBLEM: str, ORDER: str) -> None:
    """
