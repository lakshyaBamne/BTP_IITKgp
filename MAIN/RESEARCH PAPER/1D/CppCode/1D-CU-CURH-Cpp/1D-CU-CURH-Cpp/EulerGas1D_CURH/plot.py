...
def plot():
    ...
    for ax in axd:
        ...
        axd[ax].plot(grid1, vars1[ind][-1], label=f"{ax} - CU Plot", color="red")
        # axd[ax].plot(grid2, vars2[ind][-1], label=f"{ax} - Reference", color="blue")
...
def animate():
    ...
    density = get_vars("result/Density.txt")

    fig, ax = plt.subplots(figsize=(10,10))
    ax.axis([0,1,-0.5,3])

    y = [] # this list is updated with new data at each iteration

    counter = count(0,1)
    def updater(i):
        ax.clear()

        ax.plot(grid, density[0], label="Initial", color="black")

        idx = next(counter)

        y.clear()
        for e in density[idx]:
            y.append(e)

        ax.plot(grid, y, label="updated",color="red")
        ax.legend()

    ani = FuncAnimation(fig=fig, func=updater, interval=1, frames=len(density)-2)
    ani.save(f"plots/{env['problem']}_density.gif")
