/*
    Code for 1st Order Local Lax Friedrichs Scheme (Rusanov Scheme)
*/
...
vvd get_llf_flux(vvd& U, vvd& F, vd& u, vd& a, double dt);
...
void update_conserved_variables(vvd& U, vd& rho, vd& u, vd& p, vd& a, double dx, double& dt){
    ...
    vvd LLF = get_llf_flux(U, F, u, a, dt);

    //! Update the variables using the calculated fluxes using Euler Forward Differences
    for(int i=1 ; i<=CTS::N ; i++){
        U[0][i] = U[0][i] - (dt/CTS::dx)*(LLF[0][i]-LLF[0][i-1]);
        U[1][i] = U[1][i] - (dt/CTS::dx)*(LLF[1][i]-LLF[1][i-1]);
        U[2][i] = U[2][i] - (dt/CTS::dx)*(LLF[2][i]-LLF[2][i-1]);
    }
